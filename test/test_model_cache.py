...
class TestModelCache:
    ...
    def test_download_and_save_from_mlflow(self, tmp_path):
        ...
        with requests_mock.Mocker() as mocked_req:
            mocked_req.get(
                "http://test-tracking-uri:8080/get-artifact?path=full_model.pkl&run_uuid=123-abc",
                text="Hello World",
            )
    ...
    def test_return_a_couple_models(self):
        ...
        def get_search_return_values(experiment_ids, *args, **kwargs):
            df = pd.DataFrame(
                columns=list(cache.columns_of_interest.keys()),
                data=[
                    {
                        "run_id": "123",
        ...
        assert [
            x["is_latest"]
            for x in available_models["groceries"]
            if x["run_id"] == "456"
        ][0]
        assert not [
            x["is_latest"]
    ...
    def test_get_model_by_name(self, tmp_path):
        ...
        with requests_mock.Mocker() as mocked_req:
            mocked_req.get(
                "http://test-tracking-uri:8080/get-artifact?path=full_model.pkl&run_uuid=123",
                content=model_as_bytes,
            )
            mlflow.get_experiment_by_name = MagicMock(side_effect=get_experiment_id)
            mlflow.search_runs = MagicMock(side_effect=get_search_return_values)
            loaded_model = cache.get_loaded_model_for_scenario_and_run_id(
                "groceries", "123"
            )
    ...
    def test_get_latest_model_with_latest(self, tmp_path):
        ...
        with requests_mock.Mocker() as mocked_req:
            mocked_req.get(
                "http://test-tracking-uri:8080/get-artifact?path=full_model.pkl&run_uuid=456",
                content=model_as_bytes,
            )
            mlflow.get_experiment_by_name = MagicMock(side_effect=get_experiment_id)
            mlflow.search_runs = MagicMock(side_effect=get_search_return_values)
