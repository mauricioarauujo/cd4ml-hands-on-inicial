...
def get_feature_set(identifier_field, target_field, info):
    return get_generic_feature_set(
        identifier_field, target_field, info, __file__, FeatureSet
    )
...
class FeatureSet(FeatureSetBase):
    ...
    def derived_features_categorical(self, base_features):
        ...
        product_class = ff.item_nbr_to_product_class(
            base_features, self.info["item_nbr_lookup"]
        )
        product_family = ff.item_nbr_to_product_family(
            base_features, self.info["item_nbr_lookup"]
        )
        ...
        return {
            k: features[k]
            for k in self.params["derived_categorical_n_levels_dict"].keys()
        }
    ...
    def derived_features_numerical(self, base_features):
        ...
        days_until_end = ff.date_to_days_until_end(
            base_features, self.info["date_lookup"]
        )

        features = {"day": day, "day_off": day_off, "days_until_end": days_until_end}
