...
class Problem(ProblemBase):
    def __init__(
        ...
        self.training_filter, self.validation_filter = get_training_validation_filters(
            self.ml_pipeline_params
        )

    def prepare_feature_data(self):
        self.logger.info("Preparing feature data")
        train_data = self.training_stream()
        date_lookup = create_lookup(
            train_data, ["dayofweek", "days_til_end_of_data", "dayoff"], "date"
        )
    ...
    def get_feature_set_constructor(feature_set_name):
        if feature_set_name == "default":
            from cd4ml.problems.groceries.features.feature_sets.default import (
                feature_set as fs,
            )
            ...
            raise ValueError(
                "feature_set_name '{}' is invalid. Please check your groceries configuration".format(
                    feature_set_name
                )
            )
        pass
