...
ERROR_NO_AVAILABLE_LATEST_MODEL = (
    "No model is available for the latest scenario {}. "
    "Please re-run jenkins and reload this page"
)
ERROR_NO_MODEL_AT_LOCATION = (
    "No model is available for this identifier scenario {}. "
...
def make_page_for_scenario_and_identifier(scenario_name, identifier, request_data):
    ...
    header_text, form_div, prediction = get_form_from_model(
        scenario_name, identifier, model, initial_values=request_data
    )
...
def check_that_page_can_be_loaded(scenario_name, identifier):
    try:
        return (
            cache.get_loaded_model_for_scenario_and_run_id(scenario_name, identifier)
            is not None
        )
    except requests.exceptions.ConnectionError as e:
...
def call_model_for_scenario_and_identifier(scenario_name, identifier=None):
    ...
    _, _, prediction = make_page_for_scenario_and_identifier(
        scenario_name, identifier, json_body
    )
    ...
    return (
        prediction if prediction is not None else {"error": "No Model Available To Use"}
    )
    ...
    if len(form_data) == 0:
        ...
        if has_backing_model is False:
            message = (
                ...
                else ERROR_NO_MODEL_AT_LOCATION
            ).format(scenario_name)
            return render_template(
                "error.html", title="No model available to use", message=message
            )
        form_data = None

    # There is a model, run and return the output
    identifier_to_use = "latest" if identifier is None else identifier
    _, form_div, prediction = make_page_for_scenario_and_identifier(
        scenario_name, identifier_to_use, form_data
    )
