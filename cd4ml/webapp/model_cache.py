...
class ModelCache:
    ...
    def _get_id_for_latest_model(self, all_models_for_scenario):
        possible_deployable_models = [
            row
            for row in all_models_for_scenario
            if self.is_latest_deployable_model(row)
        ]
        if len(possible_deployable_models) == 0:
            return None

        last_deployment_model = sorted(
            possible_deployable_models,
    ...
    def get_loaded_model_for_scenario_and_run_id(self, scenario, run_id):
        if run_id == "latest":
            all_models_for_scenario = self.list_available_models_from_ml_flow().get(
                scenario
            )
            ...
            latest_item = [
                item for item in all_models_for_scenario if item["is_latest"]
            ]
            if len(latest_item) == 0:
                return None
            return self.get_loaded_model_for_scenario_and_run_id(
                scenario, latest_item[0]["run_id"]
            )
    ...
    def list_available_models_from_ml_flow(self):
        ...
        for scenario in self.known_problems:
            ...
            dataframe_with_columns_of_interest = runs[
                list(self.columns_of_interest.keys())
            ]
            dataframe_with_columns_renamed = dataframe_with_columns_of_interest.rename(
                columns=self.columns_of_interest
            )
            dataframe_with_columns_renamed["time"] = dataframe_with_columns_renamed[
                "time"
            ].dt.strftime("%c")
    ...
    def download_and_save_from_ml_flow(path, run_id):
        ...
        results = requests.get(
            "{}/get-artifact?path=full_model.pkl&run_uuid={}".format(
                mlflow.get_tracking_uri(), run_id
            )
        )
    ...
    def is_latest_deployable_model(row):
        return (
            row["ml_pipeline_params_name"] == "default"
            and row["feature_set_name"] == "default"
            and row["algorithm_name"] == "default"
            and row["algorithm_params_name"] == "default"
            and row["passed_acceptance_test"] == "yes"
        )
